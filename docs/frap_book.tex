...
Type $\mathsf{t}(\alpha)$ stands for queues holding data values in some type $\alpha$.
\begin{eqnarray*}
...
We declare that $\mathsf{t}(\alpha)$ is a type by assigning it the type $\mathsf{Set}$, which itself contains all the normal types of programming.
An empty queue exists for any $\alpha$, and enqueue and dequeue operations are also available for any $\alpha$.
The type of $\mathsf{dequeue}$ indicates function partiality\index{partial function} by the arrow $\rightharpoonup$: dequeuing yields no answer for an empty queue.
...
$$\infer{\mathsf{dequeue}(\mathsf{empty}) = \cdot}{}
\quad \infer{q = \mathsf{empty}}{\mathsf{dequeue}(q) = \cdot}$$
...
$$\infer{\mathsf{dequeue}(\mathsf{enqueue}(q, x)) = \begin{cases}
...
We write $\mathsf{list}(\alpha)$ for the type of lists\index{lists} with data elements from $\alpha$, with $\concat{\ell_1}{\ell_2}$ for concatenation of lists $\ell_1$ and $\ell_2$, and with comma-separated lists inside square brackets for list literals.
\begin{eqnarray*}
  \mathsf{t(\alpha)} &=& \mathsf{list}(\alpha) \\
  \mathsf{empty} &=& [] \\
  \mathsf{enqueue}(q, x) &=& \concat{[x]}{q} \\
  \mathsf{dequeue}([]) &=& \cdot \\
  \mathsf{dequeue}(\concat{[x]}{q}) &=& ([], x)\textrm{, when $\mathsf{dequeue}(q) = \cdot$.} \\
  \mathsf{dequeue}(\concat{[x]}{q}) &=& (\concat{[x]}{q'}, y)\textrm{, when $\mathsf{dequeue}(q) = (q', y)$.}
\end{eqnarray*}
...
\begin{eqnarray*}
  \mathsf{t(\alpha)} &=& \mathsf{list}(\alpha) \\
  \mathsf{empty} &=& [] \\
  \mathsf{enqueue}(q, x) &=& \concat{q}{[x]} \\
  \mathsf{dequeue}([]) &=& \cdot \\
  \mathsf{dequeue}(\concat{[x]}{q}) &=& (q, x)
\end{eqnarray*}
...
We implicitly lift $\approx$ to apply to results of the partial function $\mathsf{dequeue}$: nonexistent results $\cdot$ are related, and existent results $(q_1, x_1)$ and $(q_2, x_2)$ are related iff $q_1 \approx q_2$ and $x_1 = x_2$.
$$\infer{\mathsf{dequeue}(\mathsf{empty}) = \cdot}{}
\quad \infer{q \approx \mathsf{empty}}{\mathsf{dequeue}(q) = \cdot}$$

$$\infer{\mathsf{dequeue}(\mathsf{enqueue}(q, x)) \approx \begin{cases}
    (\mathsf{empty}, x), & \mathsf{dequeue}(q) = \cdot \\
    (\mathsf{enqueue}(q', x), y), & \mathsf{dequeue}(q) = (q', y)
  \end{cases}}{}$$
...
Here is its implementation, relying on list-reversal function $\mathsf{rev}$ (which takes linear time).
\begin{eqnarray*}
  \mathsf{t(\alpha)} &=& \mathsf{list}(\alpha) \times \mathsf{list}(\alpha) \\
  \mathsf{empty} &=& ([], []) \\
  \mathsf{enqueue}((\ell_1, \ell_2), x) &=& (\concat{[x]}{\ell_1}, \ell_2) \\
  \mathsf{dequeue}(([], [])) &=& \cdot \\
  \mathsf{dequeue}((\ell_1, \concat{[x]}{\ell_2})) &=& ((\ell_1, \ell_2), x) \\
  \mathsf{dequeue}((\ell_1, [])) &=& (([], q'_1), x)\textrm{, when $\mathsf{rev}(\ell_1) = \concat{[x]}{q'_1}$.}
\end{eqnarray*}
...
\begin{eqnarray*}
  \mathsf{rep}((\ell_1, \ell_2)) &=& \concat{\ell_1}{\mathsf{rev}(\ell_2)} \\
...
$$\infer{\mathsf{rep}(\mathsf{empty}) = []}{}
\quad \infer{\mathsf{rep}(\mathsf{enqueue}(q, x)) = \concat{[x]}{\mathsf{rep}(q)}}{}$$

$$\infer{\mathsf{dequeue}(q) = \cdot}{\mathsf{rep}(q) = []}
\quad \infer{\exists q'. \; \mathsf{dequeue}(q) = (q', x) \land \mathsf{rep}(q') = \ell}{\mathsf{rep}(q) = \concat{\ell}{[x]}}$$

Notice that this specification style can also be viewed as \emph{giving a reference implementation\index{reference implementations of data types} of the data type}, where $\mathsf{rep}$ shows how to convert back to the reference implementation at any point.
...
$$\infer{\mathsf{member}(\mathsf{empty}, k) = \bot}{}
\quad \infer{\mathsf{member}(\mathsf{add}(s, k), k) = \top}{}
\quad \infer{\mathsf{member}(\mathsf{add}(s, k_1), k_2) = \mathsf{member}(s, k_2)}{k_1 \neq k_2}$$

There is a simple generic implementation of this data type with unsorted lists.
\begin{eqnarray*}
  \mathsf{t} &=& \mathsf{list} \\
  \mathsf{empty} &=& [] \\
  \mathsf{add}(s, k) &=& \concat{[k]}{s} \\
  \mathsf{member}([], k) &=& \bot \\
  \mathsf{member}(\concat{[k']}{s}, k) &=& k = k' \lor \mathsf{member}(s, k)
\end{eqnarray*}
...
Assume a fallback implementation of finite sets, with type $t_0$ and operations $\mathsf{empty}_0$, $\mathsf{add}_0$, and $\mathsf{member}_0$.
We implement our optimized set type like so, assuming an operation $\mathsf{fromRange} : \mathbb N \times \mathbb N \to \mathsf{t}_0$ to turn a range into an ad-hoc set.
\begin{eqnarray*}
  ...
  \mathsf{add}(\mathsf{Range}(n_1, n_2), k) &=& \mathsf{Range}(n_1, n_2)\textrm{, when $n_1 \leq k \leq n_2$} \\
  \mathsf{add}(\mathsf{Range}(n_1, n_2), n_1-1) &=& \mathsf{Range}(n_1-1, n_2)\textrm{, when $n_1 \leq n_2$} \\
  \mathsf{add}(\mathsf{Range}(n_1, n_2), n_2+1) &=& \mathsf{Range}(n_1, n_2+1)\textrm{, when $n_1 \leq n_2$} \\
  \mathsf{add}(\mathsf{Range}(n_1, n_2), k) &=& \mathsf{AdHoc}(\mathsf{add}_0(\mathsf{fromRange}(n_1, n_2), k))\textrm{, otherwise} \\
...
\begin{eqnarray*}
  \mathsf{foo} &=& \lambda(x, y). \; \elet{u}{x + y}{\elet{v}{u \times y}{u + v}}
\end{eqnarray*}
...
\begin{eqnarray*}
  \mathsf{Const} &:& \mathbb N \to \mathsf{exp} \\
  \mathsf{Var} &:& \mathbb V \to \mathsf{exp} \\
  \mathsf{Plus} &:& \mathsf{exp} \to \mathsf{exp} \to \mathsf{exp} \\
  \mathsf{Times} &:& \mathsf{exp} \to \mathsf{exp} \to \mathsf{exp} \\
  \mathsf{Let} &:& \mathbb V \to \mathsf{exp} \to \mathsf{exp} \to \mathsf{exp}
\end{eqnarray*}

That last example program, with implicit \emph{free variables}\index{free variables} $x$ and $y$, may now be redefined in the $\mathsf{exp}$ type.
\newcommand{\var}[1]{\mathsf{Var} \; \textrm{``#1''}}
\begin{eqnarray*}
  \mathsf{foo'} &=& \mathsf{Let} \; (\var{u}) \; (\mathsf{Plus} \; (\var{x}) \; (\var{y})) \; (\mathsf{Let} \; (\var{v}) \\
  && \hspace{.1in} (\mathsf{Times} \; (\var{u}) \; (\var{y})) \; (\mathsf{Plus} \; (\var{u}) \; (\var{v})))
\end{eqnarray*}

As in Chapter \ref{interpreters}, we can define a recursive interpreter, mapping $\mathsf{exp}$ programs and variable valuations to numbers.
Using that interpreter, we can prove equivalence of $\mathsf{foo}$ and $\mathsf{foo'}$.

We say that $\mathsf{foo}$ uses a \emph{shallow embedding}\index{shallow embedding}, because it is coded directly in the metalanguage, with no extra layer of syntax.
Conversely, $\mathsf{foo'}$ uses a \emph{deep embedding}\index{deep embedding}, since it goes via the inductively defined $\mathsf{exp}$ type.
...
Here's an example, as an alternative definition of $\mathsf{exp}$.
\encoding
\begin{eqnarray*}
  \mathsf{Const} &:& \mathbb N \to \mathsf{exp} \\
  \mathsf{Var} &:& \mathbb V \to \mathsf{exp} \\
  \mathsf{Plus} &:& \mathsf{exp} \to \mathsf{exp} \to \mathsf{exp} \\
  \mathsf{Times} &:& \mathsf{exp} \to \mathsf{exp} \to \mathsf{exp} \\
  \mathsf{Let} &:& \mathsf{exp} \to (\mathbb N \to \mathsf{exp}) \to \mathsf{exp}
\end{eqnarray*}

The one change is in the type of the $\mathsf{Let}$ constructor, where now no variable name is given, and instead \emph{the body of the ``let'' is represented as a Gallina function from numbers to expressions}.
...
\begin{eqnarray*}
  \mathsf{foo''} &=& \mathsf{Let} \; (\mathsf{Plus} \; (\var{x}) \; (\var{y})) \; (\lambda u. \\
  && \hspace{.1in} \mathsf{Let} \; (\mathsf{Times} \; (\mathsf{Const} \; u) \; (\var{y})) \; (\lambda v. \\
  && \hspace{.2in} \mathsf{Plus} \; (\mathsf{Const} \; u) \; (\mathsf{Const} \; v)))
\end{eqnarray*}

With a bit of subtlety, we can define an interpreter for this language, too.
\begin{eqnarray*}
  \denote{\mathsf{Const} \; n}v &=& n \\
  \denote{\mathsf{Var} \; x}v &=& \msel{v}{x} \\
  \denote{\mathsf{Plus} \; e_1 \; e_2}v &=& \denote{e_1}v + \denote{e_2}v \\
  \denote{\mathsf{Times} \; e_1 \; e_2}v &=& \denote{e_1}v \times \denote{e_2}v \\
  \denote{\mathsf{Let} \; e_1 \; e_2}v &=& \denote{e_2(\denote{e_1}v)}v
\end{eqnarray*}

Note how, in the $\mathsf{Let}$ case, since the body $e_2$ is a function, before evaluating it, we call it on the result of evaluating $e_1$.
This language would actually be sufficient even if we removed the $\mathsf{Var}$ constructor and the $v$ argument of the interpreter.
...
The second argument of $\mathsf{Let}$ above is an example.
When we do induction on expression syntax to establish $\forall e. \; P(e)$, the case for $\mathsf{Let} \; e_1 \; e_2$ includes two induction hypotheses.
...
We can define a polymorphic type family $\mathsf{cmd}$ of commands, indexed by the type of value that a command is meant to return.
\encoding
\begin{eqnarray*}
  \mathsf{Return} &:& \forall \alpha. \; \alpha \to \mathsf{cmd} \; \alpha \\
  \mathsf{Bind} &:& \forall \alpha, \beta. \; \mathsf{cmd} \; \beta \to (\beta \to \mathsf{cmd} \; \alpha) \to \mathsf{cmd} \; \alpha \\
  \mathsf{Read} &:& \mathbb N \to \mathsf{cmd} \; \mathbb N \\
  \mathsf{Write} &:& \mathbb N \to \mathbb N \to \mathsf{cmd} \; \mathsf{unit}
\end{eqnarray*}

We use notation $x \leftarrow c_1; c_2$ as shorthand for $\mathsf{Bind} \; c_1 \; (\lambda x. \; c_2)$, making it possible to write some very natural-looking programs in this type.
Here are two examples.
\begin{eqnarray*}
  \mathsf{array\_max}(0, a) &=& \mathsf{Return} \; a \\
  \mathsf{array\_max}(i+1, a) &=& v \leftarrow \mathsf{Read} \; i; \mathsf{array\_max} \; i \; (\max(v, a)) \\
  \\
  \mathsf{increment\_all}(0) &=& \mathsf{Return} \; () \\
  \mathsf{increment\_all}(i+1) &=& v \leftarrow \mathsf{Read} \; i; \_ \leftarrow \mathsf{Write} \; i \; (v+1); \mathsf{increment\_all} \; i
\end{eqnarray*}

Function $\mathsf{array\_max}$ computes the highest value found in the first $i$ slots of memory, using an accumulator $a$.
Function $\mathsf{increment\_all}$ adds 1 to every one of the first $i$ memory slots.
...
We are able to do so despite the fact that we built no support for recursion into the $\mathsf{cmd}$ type family.
...
\begin{eqnarray*}
  \denote{\mathsf{Return} \; v}h &=& (h, v) \\
  \denote{\mathsf{Bind} \; c_1 \; c_2}h &=& \elet{(h', v)}{\denote{c_1}h}{\denote{c_2(v)}h'} \\
  \denote{\mathsf{Read} \; a}h &=& (h, \msel{h}{a}) \\
  \denote{\mathsf{Write} \; a \; v}h &=& (\mupd{h}{a}{v}, ())
\end{eqnarray*}
...
$$\infer{\hoare{P}{\mathsf{Return} \; v}{\lambda r, h. \; P(h) \land r = v}}{}
\quad \infer{\hoare{P}{\mathsf{Bind} \; c_1 \; c_2}{R}}{
  \hoare{P}{c_1}{Q}
  & \forall r. \; \hoare{Q(r)}{c_2(r)}{R}
}$$
$$\infer{\hoare{P}{\mathsf{Read} \; a}{\lambda r, h. \; P(h) \land r = \msel{h}{a}}}{}
\quad \infer{\hoare{P}{\mathsf{Write} \; a \; v}{\lambda r, h. \; \exists h'. \; P(h') \land h = \mupd{h'}{a}{v}}}{}$$
$$\infer{\hoare{P'}{c}{Q'}}{
  \hoare{P}{c}{Q}
...
The most interesting new wrinkle is in the rule for $\mathsf{Bind}$, where the premise about the body command $c_2$ starts with universal quantification over all possible results $r$ of executing $c_1$.
...
For a type $\alpha$, we define $\mathbb O(\alpha)$ as the type of \emph{loop-body outcomes}, either $\mathsf{Done}(a)$ to indicate that the loop should terminate or $\mathsf{Again}(a)$ to indicate that the loop should keep running.
...
\begin{eqnarray*}
  \mathsf{Loop} &:& \forall \alpha. \; \alpha \to (\alpha \to \mathsf{cmd} \; (\mathbb O(\alpha))) \to \mathsf{cmd} \; \alpha
\end{eqnarray*}
...
\begin{eqnarray*}
  \mathsf{index\_of}(n) &=& \mathsf{Loop} \; 0 \; (\lambda i. \; v \leftarrow \mathsf{Read} \; i; \mathsf{if} \; v = n \; \mathsf{then} \; \mathsf{Return} \; (\mathsf{Done}(i)) \; \mathsf{else} \; \mathsf{Return} \; (\mathsf{Again}(i+1)))
\end{eqnarray*}
...
$$\infer{\hoare{I(\mathsf{Again}(i))}{\mathsf{Loop} \; i \; c}{\lambda r. \; I(\mathsf{Done}(r)))}}{
  \forall a. \; \hoare{I(\mathsf{Again}(a))}{c(a)}{I}
}$$
...
\invariants
\begin{theorem}
  If $\hoare{P}{c}{Q}$ and $P(h)$ for some heap $h$, then it is an invariant of $(c, h)$ that, if the command ever becomes $\mathsf{Return} \; r$ in a heap $h'$, then $Q(r, h')$.  That is, if the program terminates, the postcondition is satisfied.
\end{theorem}
...
\begin{eqnarray*}
  \mathsf{Fail} &:& \forall \alpha. \; \mathsf{cmd} \; \alpha
\end{eqnarray*}
...
The operational semantics is also easily extended to signal failures, with a new special system state called $\mathsf{Failed}$.
We also add this Hoare-logic rule.
$$\infer{\hoare{\lambda \_. \; \bot}{\mathsf{Fail}}{\lambda \_, \_. \; \bot}}{}$$
...
\invariants
\begin{theorem}
  If $\hoare{P}{c}{Q}$ and $P(h)$ for some heap $h$, then it is an invariant of $(c, h)$ that the state never becomes $\mathsf{Failed}$.
\end{theorem}

Note that this version of the theorem still tells us interesting things about programs that run forever.
It is easy to implement runtime assertion checking with code that performs some test and runs $\mathsf{Fail}$ if the test does not pass.
...
$$\begin{array}{rrcl}
  \textrm{Commands} & c &::=& \mathsf{Return} \; v \mid x \leftarrow c; c \mid \mathsf{Loop} \; i \; f \mid \mathsf{Fail} \\
  &&& \mid \mathsf{Read} \; n \mid \mathsf{Write} \; n \; n \mid \mathsf{Alloc} \; n \mid \mathsf{Free} \; n \; n
\end{array}$$
...
\quad \infer{\smallstep{(h, x \leftarrow \mathsf{Return} \; v; c(x))}{(h, c(v))}}{}$$

$$\infer{\smallstep{(h, \mathsf{Loop} \; i \; f)}{(h, x \leftarrow f(i); \mathsf{match} \; x \; \mathsf{with} \; \mathsf{Done}(a) \Rightarrow \mathsf{Return} \; a \mid \mathsf{Again}(a) \Rightarrow \mathsf{Loop} \; a \; f)}}{}$$

$$\infer{\smallstep{(h, \mathsf{Read} \; a)}{(h, \mathsf{Return} \; v)}}{
  \msel{h}{a} = v
}
\quad \infer{\smallstep{(h, \mathsf{Write} \; a \; v')}{(\mupd{h}{a}{v'}, \mathsf{Return} \; ())}}{
  \msel{h}{a} = v
}$$

$$\infer{\smallstep{(h, \mathsf{Alloc} \; n)}{(\mupd{h}{a}{0^n}, \mathsf{Return} \; a)}}{
  \dom{h} \cap [a, a+n) = \emptyset
}
\quad \infer{\smallstep{(h, \mathsf{Free} \; a \; n)}{(h - [a, a+n), \mathsf{Return} \; ())}}{
}$$

A few remarks about the last four rules:
The basic $\mathsf{Read}$ and $\mathsf{Write}$ operations now get \emph{stuck} when accessing unmapped addresses.
The premise of the rule for $\mathsf{Alloc}$ enforces that address $a$ denotes a currently unmapped memory region of size $n$.
...
Similarly, the conclusion of the $\mathsf{Free}$ rule unmaps a whole size-$n$ region, starting at $a$.
...
$$\infer{\hoare{P}{\mathsf{Return} \; v}{\lambda r. \; P * \lift{r = v}}}{}
\quad \infer{\hoare{P}{x \leftarrow c_1; c_2(x)}{R}}{
  \hoare{P}{c_1}{Q}
  & (\forall r. \; \hoare{Q(r)}{c_2(r)}{R})
}$$

$$\infer{\hoare{I(\mathsf{Again}(i))}{\mathsf{Loop} \; i \; f}{\lambda r. \; I(\mathsf{Done}(r))}}{
  \forall a. \; \hoare{I(\mathsf{Again}(a))}{f(a)}{I}
}
\quad \infer{\hoare{\lift{\bot}}{\mathsf{Fail}}{\lambda \_. \; \lift{\bot}}}{}$$
...
First, we have the rule for $\mathsf{Read}$.

$$\infer{\hoare{\exists v. \; \ptsto{a}{v} * R(v)}{\mathsf{Read} \; a}{\lambda r. \; \ptsto{a}{r} * R(r)}}{}$$
...
$$\infer{\hoare{\exists v. \; \ptsto{a}{v}}{\mathsf{Write} \; a \; v'}{\lambda \_. \; \ptsto{a}{v'}}}{}$$

This last rule, for $\mathsf{Write}$, is even simpler.
We see a straightforward illustration of overwriting $a$'s old value $v$ with the new value $v$'.

$$\infer{\hoare{\emp}{\mathsf{Alloc} \; n}{\lambda r. \; \ptsto{r}{0^n}}}{}
\quad \infer{\hoare{\ptsto{a}{\; ?^n}}{\mathsf{Free} \; a \; n}{\lambda \_. \; \emp}}{}$$
...
It is possible to define (recursively) a predicate $\mathsf{llist}(\ell, p)$, capturing the idea that the heap contains exactly an imperative linked list, rooted at pointer $p$, representing functional linked list $\ell$.
We can also prove a general specification for a list-reversal function:
$$\forall \ell, p. \; \hoare{\mathsf{llist}(\ell, p)}{\texttt{reverse}(p)}{\lambda r. \; \mathsf{llist}(\mathsf{rev}(\ell), r)}$$

Now consider that we have the roots $p_1$ and $p_2$ of two disjoint lists, respectively representing $\ell_1$ and $\ell_2$.
It is easy to instantiate the general theorem and get $\hoare{\mathsf{llist}(\ell_1, p_1)}{\texttt{reverse}(p_1)}{\lambda r. \; \mathsf{llist}(\mathsf{rev}(\ell_1), r)}$ and $\hoare{\mathsf{llist}(\ell_2, p_2)}{\texttt{reverse}(p_2)}{\lambda r. \; \mathsf{llist}(\mathsf{rev}(\ell_2), r)}$.
Applying the frame rule to the former theorem, with $R = \mathsf{llist}(\ell_2, p_2)$, we get:
$$\hoare{\mathsf{llist}(\ell_1, p_1) * \mathsf{llist}(\ell_2, p_2)}{\texttt{reverse}(p_1)}{\lambda r. \; \mathsf{llist}(\mathsf{rev}(\ell_1), r) * \mathsf{llist}(\ell_2, p_2)}$$
Similarly, applying the frame rule to the latter, with $R = \mathsf{llist}(\mathsf{rev}(\ell_1), r)$, we get:
$$\hoare{\mathsf{llist}(\ell_2, p_2) * \mathsf{llist}(\mathsf{rev}(\ell_1), r)}{\texttt{reverse}(p_2)}{\lambda r'. \; \mathsf{llist}(\mathsf{rev}(\ell_2), r') * \mathsf{llist}(\mathsf{rev}(\ell_1), r)}$$
Now it is routine to derive the following spec for a larger program:
$$\begin{array}{l}
  \{\mathsf{llist}(\ell_1, p_1) * \mathsf{llist}(\ell_2, p_2)\} \\
  \hspace{.2in} r \leftarrow \texttt{reverse}(p_1); r' \leftarrow \texttt{reverse}(p_2); \; \mathsf{Return}(r, r') \\
  \{\lambda (r, r'). \; \mathsf{llist}(\mathsf{rev}(\ell_1), r) * \mathsf{llist}(\mathsf{rev}(\ell_2), r')\}
\end{array}$$
...
The proof internals require only the basic rules for $\mathsf{Return}$ and sequencing, in addition to the rule of consequence, whose side conditions we discharge using the cancellation approach sketched in the previous section.
...
\invariants
\begin{theorem}
  If $\hoare{P}{c}{Q}$ and $P(\mempty)$, then it is an invariant of the transition system starting from $(\mempty, c)$ that either the command has become a $\mathsf{Return}$ or another execution step is possible.
\end{theorem}
...
\begin{lemma}[Progress]
  If $\hoare{P}{c}{Q}$ and $P(h_1)$, then either $c$ is a $\mathsf{Return}$ or it is possible to take a step from $(h_1 \uplus h_2, c)$, for any disjoint $h_2$.
\end{lemma}
\begin{proof}
  By induction on the derivation of $\hoare{P}{c}{Q}$.
\end{proof}
...
For instance, here is the one we prove for $\mathsf{Write}$.

\begin{lemma}
  If $\hoare{P}{\mathsf{Write} \; a \; v'}{Q}$, then there exists $R$ such that:
  \begin{itemize}
...
\end{lemma}
\begin{proof}
  By induction on the derivation of $\hoare{P}{\mathsf{Write} \; a \; v'}{Q}$.
\end{proof}
...
$$\begin{array}{rrcl}
  \textrm{Commands} & c &::=& \mathsf{Return} \; v \mid x \leftarrow c; c \mid \mathsf{Loop} \; i \; f \mid \mathsf{Read} \; n \mid \mathsf{Write} \; n \; n
\end{array}$$
...
We can't just write a compiler as a Gallina function, because the $\mathsf{Bind}$ construct is encoded using functions of the metalanguage.
...
The rule for lone $\mathsf{Return}$ commands is simple, delegating most of the work to expression compilation, using a designated variable \texttt{result} to hold the final answer of a command.

$$\infer{\dscomp{v}{\mathsf{Return} \; n}{\assign{\texttt{result}}{e}}}{
  \dscomp{v}{n}{e}
}$$

The most interesting rules cover uses of $\mathsf{Bind}$ on various primitive operations directly.
Here is the simplest such rule, where the primitive is simple $\mathsf{Return}$.

$$\infer{\dscomp{v}{x \leftarrow \mathsf{Return} \; n; c(x)}{\assign{y}{e}; s}}{
    y \notin \dom{v}
    & \dscomp{v}{n}{e}
...
(Actually, the source-program $\mathsf{Bind}$ is encoded as a call to a higher-order function, so no choice of a particular $x$ is present.)
...
The statement $s$ needs to represent the $\mathsf{Bind}$ body $c$, \emph{for any value $w$ that $n$ might evaluate to}.
...
Most numeric expressions in a source program will depend on results of earlier $\mathsf{Bind}$ operations.
...
In other words, if we tried to use the constant rule directly to prove the quantified premise of that last rule, when the body involves a $\mathsf{Return}$ of a complex expression that mentions the bound variable, we would generate $s$ that includes $w$, which is not allowed.

Similar rules are present for $\mathsf{Read}$ and $\mathsf{Write}$, which interestingly are compiled the same way as $\mathsf{Return}$.
...
\begin{proof}
  ...
  $$\infer{\dscomp{v}{x \leftarrow \mathsf{Return} \; n; c(x)}{\mathsf{skip}; s}}{
    v(y) = n
    & \dscomp{v}{c(n)}{s}
  }$$
...
A specification is naturally considered as a relation $R$ between inputs and outputs, where the set-based version of the specification for inputs $x$ is $\mathsf{spec}(x) = \{y \mid x \; R \; y\}$.
An implementation is naturally considered as a function $f$ from an input to an output, which can be modeled with singleton sets as $\mathsf{impl}(x) = \{f(x)\}$.
...
\begin{eqnarray*}
  \mathsf{ret} &:& \forall \alpha. \; \alpha \to \mathcal P(\alpha) \\
  \mathsf{ret} &=& \lambda x. \; \{x\} \\
  \mathsf{bind} &:& \forall \alpha, \beta. \; \mathcal P(\alpha) \to (\alpha \to \mathcal P(\beta)) \to \mathcal P(\beta) \\
  \mathsf{bind} &=& \lambda c_1. \; \lambda c_2. \; \bigcup_{x \in c_1} c_2(x)
\end{eqnarray*}

We write $x \leftarrow c_1; c_2(x)$ as shorthand for $\mathsf{bind} \; c_1 \; c_2$.
...
\begin{theorem}\label{bindret}
  $\mathsf{bind} \; (\mathsf{ret} \; v) \; c \supseteq c(v)$.
\end{theorem}

\begin{theorem}\label{refine1}
  If $c_1 \supseteq c'_1$, then $\mathsf{bind} \; c_1 \; c_2 \supseteq \mathsf{bind} \; c'_1 \; c_2$.
\end{theorem}

\begin{theorem}\label{refine2}
  If $\forall x. \; c_2(x) \supseteq c'_2(x)$, then $\mathsf{bind} \; c_1 \; c_2 \supseteq \mathsf{bind} \; c_1 \; c'_2$.
\end{theorem}
...
When we descend into the first argument or second argument of a $\mathsf{bind}$, we appeal to Theorem \ref{refine1} or \ref{refine2}, respectively.
...
\begin{theorem}[Change of representation]\label{repchange}
  ...
  \item $\mathcal M'_m = \lambda s'_0, x. \; \mathcal (s, y) \leftarrow M_m(A(s'_0), x); s' \leftarrow \{s' \mid A(s') = s\}; \mathsf{ret} \; (s', y)$
  \end{enumerate}
...
\begin{theorem}[Adding a cache]
  Let $\mathcal T = \angled{\mathcal S, \mathcal C, \mathcal M_{m \in M}}$ be an ADT, and pick a method $m$ such that $\mathcal M_m = \lambda s, x. \; \mathsf{ret} \; (s, f(s))$ for some pure function $f : \mathcal S \to \mathbb N$.
  Now define $\mathcal T' = \angled{\mathcal S \times \mathbb N, \mathcal C', \mathcal M'_{m \in M}}$, where:
  \begin{enumerate}
  \item $\mathcal C' = s \leftarrow \mathcal C; \mathsf{ret} \; (s, f(s))$
  \item $\mathcal M'_m = \lambda (s, c), x. \; \mathsf{ret} \; ((s, c), c)$
  \item For $m' \neq m$, $\mathcal M'_{m'} = \lambda (s, c), x. \; \mathcal (s', y) \leftarrow M_m(s, x); c' \leftarrow \{c' \mid f(s) = c \Rightarrow f(s') = c'\}; \mathsf{ret} \; ((s', c'), y)$
  \end{enumerate}
  Then $\mathcal T \supseteq \mathcal T'$.
\end{theorem}
\begin{proof}
...
$$\begin{array}{rrcl}
  \textrm{Commands} & c &::=& \mathsf{Fail} \mid \mathsf{Return} \; v \mid x \leftarrow c; c \mid \mathsf{Read} \; a \mid \mathsf{Write} \; a \; v \mid \mathsf{Lock} \; a \mid \mathsf{Unlock} \; a \mid c || c
\end{array}$$
...
We follow the common ``threads and locks''\index{locks} style of synchronization, with commands $\mathsf{Lock} \; a$ and $\mathsf{Unlock} \; a$ for acquiring and releasing locks, respectively.
...
\quad \infer{\smallstep{(h, l, x \leftarrow \mathsf{Return} \; v; c_2(x))}{(h, l, c_2(v))}}{}$$

$$\infer{\smallstep{(h, l, \mathsf{Read} \; a)}{(h, l, \mathsf{Return} \; \msel{h}{a})}}{}
\quad \infer{\smallstep{(h, l, \mathsf{Write} \; a \; v)}{(\mupd{h}{a}{v}, l, \mathsf{Return} \; 0)}}{}$$

$$\infer{\smallstep{(h, l, \mathsf{Lock} \; a)}{(h, l \cup \{a\}, \mathsf{Return} \; 0)}}{
  a \notin l
}
\quad \infer{\smallstep{(h, l, \mathsf{Unlock} \; a)}{(h, l \setminus \{a\}, \mathsf{Return} \; 0)}}{
  a \in l
}$$
...
\begin{eqnarray*}
  (\mupd{\mempty}{0}{1}, \emptyset, n \leftarrow \mathsf{Read} \; 0; \mathsf{Write} \; 0 \; (n+1))
  &\rightarrow& (\mupd{\mempty}{0}{1}, \emptyset, n \leftarrow \mathsf{Return} \; 1; \mathsf{Write} \; 0 \; (n+1)) \\
  &\rightarrow& (\mupd{\mempty}{0}{1}, \emptyset, \mathsf{Write} \; 0 \; (1+1)) \\
  &\rightarrow& (\mupd{\mempty}{0}{2}, \emptyset, \mathsf{Return} \; 0)
\end{eqnarray*}
...
\begin{eqnarray*}
  \rl{x \leftarrow c_1; c_2(x)} &=& \rl{c_2(v)}\textrm{, when $\rl{c_1} = \mathsf{Return} \; v$} \\
  \rl{x \leftarrow c_1; c_2(x)} &=& x \leftarrow \rl{c_1}; \rl{c_2(x)}\textrm{, when $\rl{c_1}$ is not $\mathsf{Return}$} \\
...
\begin{theorem}\label{local}
  If $\mathsf{natf}$ is an invariant of $\mathbb T_L(h, l, c)$, then it is also an invariant of $\mathbb T(h, l, c)$.
\end{theorem}
\begin{proof}
...
\newcommand{\summ}[2]{\mathsf{summarize}(#1, #2)}
...
For instance, we should take advantage of the fact that any two $\mathsf{Read}$ actions commute.
...
$$\infer{\summ{\mathsf{Return} \; r}{s}}{}
\quad \infer{\summ{\mathsf{Fail}}{s}}{}
\quad \infer{\summ{x \leftarrow c_1; c_2(x)}{s}}{
    \summ{c_1}{s}
    & \forall r. \; \summ{c_2(r)}{s}
}$$

$$\infer{\summ{\mathsf{Read} \; a}{(r, w, \ell)}}{
  a \in r
}
\quad \infer{\summ{\mathsf{Write} \; a \; v}{(r, w, \ell)}}{
  a \in w
}$$

$$\infer{\summ{\mathsf{Lock} \; a}{(r, w, \ell)}}{
  a \in \ell
}
\quad \infer{\summ{\mathsf{Unlock} \; a}{(r, w, \ell)}}{
  a \in \ell
}$$
...
\begin{eqnarray*}
  \na{\mathsf{Return} \; r} &=& \mathsf{Return} \; r \\
  \na{\mathsf{Fail}} &=& \mathsf{Fail} \\
  \na{\mathsf{Read} \; a} &=& \mathsf{Read} \; a \\
  \na{\mathsf{Write} \; a \; v} &=& \mathsf{Write} \; a \; v \\
  \na{\mathsf{Lock} \; a} &=& \mathsf{Lock} \; a \\
  \na{\mathsf{Unlock} \; a} &=& \mathsf{Unlock} \; a \\
...
\newcommand{\commu}[2]{\mathsf{commutes}(#1, #2)}
\begin{eqnarray*}
  \commu{\mathsf{Return} \; \_}{\_} &=& \top \\
  \commu{\mathsf{Fail}}{\_} &=& \top \\
  \commu{\mathsf{Read} \; a}{(\_, w, \_)} &=& a \notin w \\
  \commu{\mathsf{Write} \; a \; \_}{(r, w, \_)} &=& a \notin r \cup w \\
  \commu{\mathsf{Lock} \; a}{(\_, \_, \ell)} &=& a \notin \ell \\
  \commu{\mathsf{Unlock} \; a}{(\_, \_, \ell)} &=& a \notin \ell \\
  \commu{\_}{\_} &=& \bot
\end{eqnarray*}

\newcommand{\pors}[1]{\mathsf{porSafe}(#1)}

With these ingredients, we can define a predicate $\mathsf{porSafe}$ that figures out when a state is eligible for the partial-order-reduction optimization, which is to force the first thread to run next, ignoring the other threads for now.
...
\newcommand{\tof}[2]{\mathsf{timeOf}(#1, #2)}
...
$$\infer{\tof{\mathsf{Return} \; r}{n}}{}
\quad \infer{\tof{\mathsf{Fail}}{n}}{}
\quad \infer{\tof{\mathsf{Read} \; a}{n+1}}{}
\quad \infer{\tof{\mathsf{Write} \; a \; v}{n+1}}{}$$

$$\infer{\tof{\mathsf{Lock} \; a}{n+1}}{}
\quad \infer{\tof{\mathsf{Unlock} \; a}{n+1}}{}$$
...
\abstraction
\begin{theorem}
  If $\summ{c_2}{s}$ and $\tof{c_1 || c_2}{n}$, then to prove $\mathsf{natf}$ as an invariant of $\mathbb T(h, l, c_1 || c_2)$, it suffices to prove $\mathsf{natf}$ as an invariant of $\mathbb T_C(h, l, c_1, c_2, s)$.
\end{theorem}
\begin{proof}
  ...
  These facts contradict our assumption that $\mathsf{natf}$ is an invariant of $\mathbb T_C(h, l, c_1, c_2, s)$.
\end{proof}
...
\end{lemma}
\begin{proof}
  By induction on the derivation of $(h, l, c_1 || c_2) \to^{i+1} (h', l', c')$, appealing to a few crucial lemmas, such as single-step determinism of any state in $\mathsf{nextAction}$'s domain, plus the soundness of $\mathsf{commutes}$ with respect to single steps of pairs of commands, plus the fact that single steps preserve the accuracy of summaries.
\end{proof}
...
We might formalize that notion by saying that (1) $a$ is in the lockset, and (2) the other threads, running independently, will never manage to run an $\mathsf{Unlock} \; a$ command.
...
We only need to collect the actions that other threads can run \emph{before each one reaches its first occurrence of $\mathsf{Lock} \; a$}.
The reason is that, if $c$ holds lock $a$ and hasn't run yet, no other thread can progress past its first $\mathsf{Lock} \; a$.
...
$$(\mathsf{while} \; (\mathsf{true}) \; \{ \; \mathsf{Write} \; 0 \; 0 \; \}) \; || \; (n \leftarrow \mathsf{Read} \; 1; \mathsf{Fail})$$
...
$$\begin{array}{rrcl}
  \textrm{Commands} & c &::=& \mathsf{Fail} \mid \mathsf{Return} \; v \mid x \leftarrow c; c \mid \mathsf{Loop} \; i \; f \\
  &&& \mid \mathsf{Read} \; a \mid \mathsf{Write} \; a \; v \mid \mathsf{Alloc} \; n \mid \mathsf{Free} \; a \; n \mid \mathsf{Lock} \; a \mid \mathsf{Unlock} \; a \mid c || c
\end{array}$$
...
$$\infer{\smallstep{(h, l, \mathsf{Loop} \; i \; f)}{(h, l, x \leftarrow f(i); \mathsf{match} \; x \; \mathsf{with} \; \mathsf{Done}(a) \Rightarrow \mathsf{Return} \; a \mid \mathsf{Again}(a) \Rightarrow \mathsf{Loop} \; a \; f)}}{}$$

$$\infer{\smallstep{(h, l, \mathsf{Read} \; a)}{(h, l, \mathsf{Return} \; v)}}{
  \msel{h}{a} = v
}
\quad \infer{\smallstep{(h, l, \mathsf{Write} \; a \; v')}{(\mupd{h}{a}{v'}, l, \mathsf{Return} \; ())}}{
  \msel{h}{a} = v
}$$

$$\infer{\smallstep{(h, l, \mathsf{Alloc} \; n)}{(\mupd{h}{a}{0^n}, l, \mathsf{Return} \; a)}}{
  \dom{h} \cap [a, a+n) = \emptyset
}
\quad \infer{\smallstep{(h, l, \mathsf{Free} \; a \; n)}{(h - [a, a+n), l, \mathsf{Return} \; ())}}{
}$$

$$\infer{\smallstep{(h, l, \mathsf{Lock} \; a)}{(h, l \cup \{a\}, \mathsf{Return} \; ())}}{
  a \notin l
}
\quad \infer{\smallstep{(h, l, \mathsf{Unlock} \; a)}{(h, l \setminus \{a\}, \mathsf{Return} \; ())}}{
  a \in l
}$$
...
$$\infer{\hoare{\emp}{\mathsf{Return} \; v}{\lambda r. \; \lift{r = v}}}{}
\quad \infer{\hoare{P}{x \leftarrow c_1; c_2(x)}{R}}{
  \hoare{P}{c_1}{Q}
  & (\forall r. \; \hoare{Q(r)}{c_2(r)}{R})
}$$
...
$$\infer{\hoare{\exists v. \; \ptsto{a}{v} * R(v)}{\mathsf{Read} \; a}{\lambda r. \; \ptsto{a}{r} * R(r)}}{}
\quad \infer{\hoare{\exists v. \; \ptsto{a}{v}}{\mathsf{Write} \; a \; v'}{\lambda \_. \; \ptsto{a}{v'}}}{}$$
...
$$\infer{\hoare{\emp}{\mathsf{Lock} \; a}{\lambda \_. \; \mathcal I(a)}}{
  a \in L
}
\quad \infer{\hoare{\mathcal I(a)}{\mathsf{Unlock} \; a}{\lambda \_. \; \emp}}{
  a \in L
}$$
...
\begin{eqnarray*}
  \guarded{\phi}{P} &=& \mathsf{if} \; \phi \; \mathsf{then} \; P \; \mathsf{else} \; \emp
\end{eqnarray*}
...
\begin{lemma}
  If $\hoare{P}{\mathsf{Read} \; a}{Q}$, then there exists $R$ such that $P \Rightarrow \exists v. \; \ptsto{a}{v} * R(v)$ and, for all $r$, $\ptsto{a}{r} * R(r) \Rightarrow Q(r)$.
\end{lemma}
\begin{proof}
  By induction on the derivation of $\hoare{P}{\mathsf{Read} \; a}{Q}$.
\end{proof}
...
\newcommand{\newp}[3]{\nu[#1](#2); #3}
\newcommand{\block}[2]{\mathsf{block}(#1); #2}
\newcommand{\send}[3]{!#1(#2); #3}
\newcommand{\recv}[3]{?#1(#2); #3}
\newcommand{\parl}[2]{#1 || #2}
\newcommand{\dup}[1]{\mathsf{dup}(#1)}
\newcommand{\done}[0]{\mathsf{done}}
...
\newcommand{\neverUses}[2]{\mathsf{neverUses}(#1, #2)}
